module MySvg5 exposing (main)

import Color
import Html exposing (Html)
import TypedSvg exposing (circle, defs, linearGradient, pattern, polygon, radialGradient, rect, stop, svg)
import TypedSvg.Attributes exposing (cx, cy, fill, fx, fy, id, offset, patternUnits, points, r, stopColor, stopOpacity, stroke, viewBox, x1, x2, y1, y2)
import TypedSvg.Attributes.InPx as InPx exposing (height, strokeWidth, width, x, y)
import TypedSvg.Core exposing (Svg)
import TypedSvg.Types exposing (CoordinateSystem(..), Length(..), Opacity(..), Paint(..))
